...
本项目（`solution-vault`）与 [`sim-lab`](D:\projects\sim-lab)（仿真实验室）成对工作：

| | solution-vault（本仓） | sim-lab（仿真仓） |
|---|---|---|
...
跑仿真测试（家用主机）：
  1. git pull
  2. cd ../sim-lab/engine
  3. python -m src.cli match <bom> --scene-dir ../solution-vault/scene-library/scenes
```
...
- **创建时间**：2026-07-23（原名"智能照明方案馆"，2026-07-31 重组为 solution-vault）

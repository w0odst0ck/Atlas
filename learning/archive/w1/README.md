...
> 来源：`../tech-plans/4week-awakening.md` 周计划 · 优化版（2026-08-03）
> 方式：纯仿真，零硬件（Wokwi 浏览器）
> 目标：唤醒嵌入式手感（GPIO/中断/PWM/UART/I2C），5 个 demo 拼装 = 智能灯具雏形
> 代码：实验代码在 F 仓 `../../../Forge/labs/w1/`（本目录只放文档）
...
| D4 | UART 双实例互发（loopback） | `D4-uart/` | 串口 MATCH 循环 | ✅ 留例 + 互动课 Q1-Q3（批次 4） |
| D5 | I2C 读 MPU6050 | `D5-i2c/` | 读到加速度/温度实时值 | ✅ 留例 + 互动课 Q1-Q3（批次 5） |
...
2. Wokwi 粘贴 F 仓 `../../../Forge/labs/w1/{D}/` 的 `sketch.ino` + `diagram.json`（+ `libraries.txt`），点 ▶ 跑通
3. 验证标准打勾；面试题口头答一遍
4. 飞轮收尾：读 `understand.md` → 写「我的理解」3 句到 `notes/Dn.md` → 概念卡复制进 `study-cards.md` → 填架构图空格
5. 完成状态告诉 Agent → 更新本表看板 + 同步（协议见 `../../COLLAB.md`）
...
| D | 跑通 ✅ | 理解 🔵 | 架构 🟢 | 迁移 🟣 |
|---|--------|---------|---------|---------|
| D1 | 已跑通 | ✅ 互动课完成（Q1-Q4） | 控制层已标 | ✅ 已写（quiz 批次 1） |
| D2 | 已跑通 | ✅ 互动课完成（Q1-Q4+深入一问） | 感知层待填（D5 一起） | ✅ 已写（quiz 批次 2） |
| D3 | 留例（Wokwi 弃跑） | ✅ 互动课完成（Q1-Q5，5/5） | 控制层已填（PWM 深挖） | ✅ 已写（quiz 批次 3） |
| D4 | 留例 | ✅ 互动课完成（Q1-Q3，批次 4） | 通信层待填（W2 一起） | ✅ 已写（quiz 批次 4） |
| D5 | 留例 | ✅ 互动课完成（Q1-Q3，批次 5） | 感知层待填（与 D2 一起） | ✅ 已写（quiz 批次 5） |
| D6 | 串讲 | **评审中**（08-12 首轮：必答 5 题过，串线待复述） | 全图回顾 | 评审卷已建 |
| D7 | 复盘 | 复盘 | W2 衔接 | 复盘 |
...
- 知识点 → `W1-嵌入式.md` 手册（每课一节）
- 问答+原答+批改 → `W1-quiz.md`（每批一次）
- 过程/决策/踩坑 → `W1-review.md`
- 复习调度 → `../review-checklist.md`（1/3/7 轮）

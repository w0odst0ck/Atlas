# 智能照明系统架构 · v0 总图（学习飞轮 · 方案 2）

> 定位：W1-W4 学习的"架构认知骨架"。先看懂全局，再每完成一个 D 填一格。
> 演进规则：🟡 = 留白待填（对应学习任务）｜✅ = 已填
> 配套：`../README.md` 学习飞轮｜`ADR.md` 设计决策记录｜课纲 `../../../plan/courses.md`｜计划源 `../../../tech-plans/4week-awakening.md`

## 一、五层架构总图

```mermaid
graph TB
    subgraph 应用层["应用层 · 业务逻辑"]
        A1[场景调度<br/>课表联动/人来灯亮]
        A2[节能审计<br/>能耗统计/报告]
        A3[管理平台<br/>Web/App]
    end
    subgraph 平台层["平台层 · 数据处理"]
        P1[网关<br/>协议转换/边缘逻辑]
        P2[时序库<br/>能耗数据]
        P3[MQTT Broker<br/>消息总线]
    end
    subgraph 通信层["通信层 · 设备联网"]
        C1[DALI<br/>灯具级调光]
        C2[KNX<br/>楼宇级控制]
        C3[Modbus/RS-485<br/>工控/网关]
        C4[蓝牙Mesh/Zigbee<br/>无线]
    end
    subgraph 控制层["控制层 · 执行"]
        B1[调光驱动<br/>0-10V/DALI/PWM 输入]
        B2[继电器/回路控制器]
    end
    subgraph 感知层["感知层 · 采集"]
        S1[存在型雷达<br/>静止人体检测]
        S2[光照传感器<br/>恒照度]
        S3[面板/按键]
    end

    S1 --> B1
    S2 --> B1
    S3 --> C3
    B1 --> C1
    B1 --> C3
    C1 --> P1
    C3 --> P1
    C2 --> P1
    C4 --> P1
    P1 --> P3
    P3 --> P2
    P1 --> A1
    P2 --> A2
    A1 --> A3
```

**核心认知**：智能照明 = 感知（采数据）+ 控制（执行调光）+ 通信（联网）+ 平台（数据/逻辑）+ 应用（业务）。W1-W4 就是从底层往上爬：W1 摸透控制层和感知层的"手感"，W2 打通通信层，W3 深挖调光驱动，W4 组装平台层+应用层。

## 二、W1 demo ↔ 架构位置映射

| D | demo | 架构层 | 说明 |
|---|------|--------|------|
| D1 | GPIO 点灯 | 控制层 | "灯亮/灭"的最基本执行单元。LED = 一盏灯的抽象 |
| D2 | 按键中断 | 感知层 + 控制逻辑 | 输入设备接入 + 事件驱动处理 |
| D3 | PWM 调光 | 控制层（核心） | 调光驱动的底层机制，W3 深挖上层协议 |
| D4 | UART | 通信层 | 点对点链路——Modbus 的物理基础 |
| D5 | I2C 传感器 | 感知层 | 传感器数据采集，恒照度/人感的前提 |

### 控制层深挖（D3 后已填，2026-08-11）

- **调光驱动 = PWM 三要素**：占空比（亮度旋钮）/ 频率（闪烁消除器，>60Hz 视觉暂留）/ 分辨率（感知均匀，韦伯-费希纳对数感知）
- **实现机制**：LEDC 硬件外设 offload（CPU 写一次占空比，外设自动出方波）——CPU 解放，16 通道 = 多路灯具
- **行业决策框架**：EMI 是工程问题（频率抖动/软开关/滤波器可修），色温/效率是原理问题（无解）→ 选 PWM 恒流方案
- **迁移锚点**：0-10V/DALI 骑在 PWM 上的上层协议，W3 深挖

## 三、留白格（每完成一个 D 填一格）

| 空格 | 内容 | 填格条件 | 状态 |
|------|------|----------|------|
| S2 | 恒照度闭环：光照传感器 → 调光驱动的反馈链路怎么设计？ | D3+D5 完成后 | 🟡 |
| C3 | Modbus/RS-485 为什么是网关首选协议？（可靠性/布线/成本） | W2 完成后 | 🟡 |
| C1 | DALI vs 0-10V vs 蓝牙Mesh 选型逻辑（三选一表格） | W3 完成后 | 🟡 |
| P1 | 网关职责清单：协议转换/边缘规则/断网容错 | W2+W4 完成后 | 🟡 |
| 应用层 | 一个真实场景（教室）的完整架构图 + 选型 | W4 完成后 | 🟡 |

## 四、架构理解自检（每 D 问自己）

1. 这个机制在系统图里属于哪一层？和上下层怎么协作？
2. 如果它坏了，影响范围多大？（故障域思维——架构师第一课）
3. 换一种方案做同一件事，代价是什么？（选型思维）

---

> v0 日期：2026-08-03｜后续每完成一个 D，更新此文件（Agent 执行，用户确认）

...
- **定义**：脉冲宽度调制——通过改变高电平占空比控制平均输出功率
- **两个独立维度**：占空比 → 亮度（平均光能 = 满亮 × 占空比）；频率 → 闪烁（视觉暂留下限 ~60Hz，IEEE 1789）
- **分辨率**：8bit=256 级 / 16bit=65536 级；人眼对数感知（韦伯-费希纳）→ 调光目标 = 感知均匀（gamma/对数曲线），非光强均匀
- **调光三选**：PWM 恒流（色温稳/效率 90%+/EMI 可修=工程问题）vs 模拟调流（色温漂/损耗大=原理问题）
- **ledcWrite vs digitalWrite**：硬件外设 offload（CPU 写一次占空比，外设自动出方波）vs CPU 每周期亲自翻转
- **行业**：0-10V/DALI 是上层协议，底层肌肉 = PWM/恒流驱动
